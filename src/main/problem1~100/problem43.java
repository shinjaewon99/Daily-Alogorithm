...
public class problem43 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            String reverseInputString = new StringBuffer(inputString).reverse().toString();

            // 입력예시에서 kisik는 reverse를 하여도 입력값이랑 동일하다.
            if(userInputStringList.contains(reverseInputString) || inputString.equals(reverseInputString)){
                System.out.println(inputString.length() +" " + inputString.charAt(inputString.length()/2));
                return;
            }else{
